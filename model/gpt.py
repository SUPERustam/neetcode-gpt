...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.model = nn.Sequential(*[self.TransformerBlock(model_dim=model_dim, num_heads=num_heads) for _ in range(num_blocks)], nn.LayerNorm(model_dim), nn.Linear(model_dim, vocab_size))
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        embedded += self.position_embeddings(torch.arange(context.shape[1], device=context.device))
