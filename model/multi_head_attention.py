...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        self.head_attentions = nn.ModuleList([self.SingleHeadAttention(embedding_dim, self.head_size) for _ in range(num_heads)])
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        z = list()
        for head_attention in self.head_attentions:
            z.append(head_attention(embedded))

        out = self.proj(torch.cat(z, dim=2))
